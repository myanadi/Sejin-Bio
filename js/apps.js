...
const Apps = {
  ...
  openCalendar() {
    ...
    CONTENT.schedule.thisWeek.forEach(s => {
      if (s.text) {
        html += `<div class="schedule-row">${s.day} · ${s.time} · ${s.text}</div>`;
      } else {
        html += `<div class="schedule-row schedule-empty">${s.day} · ${s.time}</div>`;
      }
    });
    html += '<div class="schedule-section-title" style="margin-top:20px;">다음 주 예정</div>';
    CONTENT.schedule.nextWeek.forEach(s => {
      html += `<div class="schedule-row">${s.day} · ${s.text}</div>`;
    });
    html += '</div>';

    this.openWindow('일정표', html, { width: 420, height: 440 });
  },
