...
const Desktop = {
  ...
  updateMailBadge() {
    ...
    if (!badge) return;
    // 안 읽은 조건부 메일 수 (터미널 진입 시 추가된 메일들)
    const unreadCount = State.data.conditionalMails.filter(
      key => {
        const mail = CONTENT.mailsConditional[key];
        return mail && !State.data.mailRead.includes(mail.id);
      }
    ).length;
    if (unreadCount > 0) {
